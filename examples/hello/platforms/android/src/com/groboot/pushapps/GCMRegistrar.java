...
class GCMRegistrar {
	...
	public static void checkManifest(Context context) {
		...
		if (Log.isLoggable("GCMRegistrar", 2)) {
			Log.v("GCMRegistrar", "number of receivers for " + packageName
					+ ": " + receivers.length);
		}
	...
	private static void checkReceiver(Context context,
		...
		if (Log.isLoggable("GCMRegistrar", 2)) {
			Log.v("GCMRegistrar", "Found " + receivers.size()
					+ " receivers for action " + action);
		}
	...
	static void internalRegister(Context context, String... senderIds) {
		...
		if (flatSenderIds == null) {
			flatSenderIds = SharedData.getInstance(context).getPrefString(
					"senderId", "");
		}
		if (flatSenderIds.length() > 0) {
			Log.v("GCMRegistrar", "Registering app " + context.getPackageName()
	...
	static String getFlatSenderIds(String[] senderIds) {
		if ((senderIds == null) || (senderIds.length == 0) || (senderIds[0] == null)) {
			return null;
		}
		...
		if (sRetryReceiver == null) {
			if (sRetryReceiverClassName == null) {
				...
				try {
					...
					Log.e("GCMRegistrar", "Could not create instance of "
	...
	static void setRetryReceiverClassName(String className) {
		Log.v("GCMRegistrar", "Setting the name of retry receiver class to "
				+ className);
	...
	public static String getRegistrationId(Context context) {
		...
		if ((oldVersion != -2147483648) && (oldVersion != newVersion)) {
			Log.v("GCMRegistrar", "App version changed from " + oldVersion
					+ " to " + newVersion + "; resetting registration id");
	...
	public static void setRegisteredOnServer(Context context, boolean flag) {
		...
		Log.v("GCMRegistrar", "Setting registeredOnServer status as " + flag
				+ " until " + new Timestamp(expirationTime));
	...
	public static boolean isRegisteredOnServer(Context context) {
		...
		if (isRegistered) {
			...
			if (System.currentTimeMillis() > expirationTime) {
				Log.v("GCMRegistrar", "flag expired on: "
